...
def autoassist(userdata):
    ...
    for zone in zones:
        try:
            iswindowopenzone = t.getOpenWindowDetected(zone["id"])["openWindowDetected"]
        except Exception as e:
            writeToLog("An Error Occurred: " + str(e))
            iswindowopenzone = False
        if iswindowopenzone:
            writeToLog("Open Window in '" + zone["name"] + "' detected: Switching to open-window-mode.")
            try:
    ...
    for device in mobileDevices:
        if device["settings"]["geoTrackingEnabled"]:
            print("HomeState: " + userdata[2] + ", Location at Home: " + str(device["location"]["atHome"]))
            if device["location"]["atHome"]: # Person zu Hause
                someonehome = True
                if (userdata[2] == "AWAY"): # System sagt niemand zu Hause
